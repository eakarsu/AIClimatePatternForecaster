...
export default function AdaptationRoadmapGeneratorFromCompanyLocationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-adaptation-roadmap-generator-from-company-location/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Adaptation-roadmap generator from company location + industry inputs</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-adaptation-roadmap-generator-from-company-location/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
