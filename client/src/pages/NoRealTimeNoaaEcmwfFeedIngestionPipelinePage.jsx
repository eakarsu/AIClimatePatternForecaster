...
export default function NoRealTimeNoaaEcmwfFeedIngestionPipelinePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-real-time-noaa-ecmwf-feed-ingestion-pipeline/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No real-time NOAA/ECMWF feed ingestion pipeline</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-real-time-noaa-ecmwf-feed-ingestion-pipeline/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
