...
export default function MultiScenarioModelingAgentGeneratingOptimistiPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-multi-scenario-modeling-agent-generating-optimisti/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Multi-scenario modeling agent generating optimistic/baseline/pessimistic impact for assets and supply chain</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-multi-scenario-modeling-agent-generating-optimisti/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
