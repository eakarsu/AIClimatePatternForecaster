...
export default function NoExportReportingModuleForEsgCdpFilingsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-export-reporting-module-for-esg-cdp-filings/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No export/reporting module for ESG/CDP filings</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-export-reporting-module-for-esg-cdp-filings/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
