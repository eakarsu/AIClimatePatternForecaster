 ...
 *
 * Project routes don't enforce auth so this follows the same convention.
 * Pass ?user_id= or send user_id in body for filtering.
 *
...
function uid(req) {
  return req.query.user_id || req.body.user_id || 'anonymous';
}
...
router.post('/', async (req, res) => {
  try {
    ...
    const target = user_id || 'anonymous';
    const t = type || 'info';
    if (await tableExists()) {
      const r = await pool.query(
...
router.post('/mark-all-read', async (req, res) => {
  try {
    const u = uid(req);
    if (await tableExists()) {
      await pool.query('UPDATE notifications SET read = true WHERE user_id = $1', [u]);
      return res.json({ success: true });
    }
    (memStore.get(u) || []).forEach(n => { n.read = true; });
...
router.delete('/:id', async (req, res) => {
  try {
    const u = uid(req);
    if (await tableExists()) {
      await pool.query('DELETE FROM notifications WHERE id = $1 AND user_id = $2', [req.params.id, u]);
      return res.json({ success: true });
    }
    const list = memStore.get(u) || [];
